 ...
 * @LastEditTime: 2021-07-15 22:31:22
 */
//启用 session
session_start();
...
$config['agent_back_url'] = esc_url(home_url('/oauth/agent/callback'));
...
if ($type == 'weixingzh' && !zib_is_wechat_app()) {
    ...
    if ($action == 'check_callback') {
        ...
        if (!empty($result['goto']) && !empty($result['option']['FromUserName'])) {
            ...
            if (!empty($_REQUEST['oauth_rurl'])) $goto_data['oauth_rurl'] = $_REQUEST['oauth_rurl'];
