...
/**
 * @description: 获取用户的超级版主身份的板块ID列表
 * @param {*} $user_id
 * @return {*}
 */
...
/**
 * @description: 获取板块查看版主或者分区版主的模态框链接
 * @param {*} $user_id
 * @param {*} $class
 * @param {*} $con
 * @return {*}
...
/**
 * @description: 当前板块的版主moderator列表查询
 * @param {*} $plate
 * @param {*} $paged
 * @return {*}
 */
...
/**
 * @description: 当前板块的版主moderator列表显示
 * @param {*} $plate
 * @param {*} $paged
 * @return {*}
 */
...
function zib_get_apply_moderator_plate_select($in_id = 0, $class = 'form-control')
{
    ...
    if ($moderator_ids && is_array($moderator_ids)) {
        $args['post__not_in'] = $moderator_ids; //排除自己已经是版主的板块
    }
    if ($plate_author_ids && is_array($plate_author_ids)) {
        $args['post__not_in'] += $plate_author_ids; //排除自己已经是超级版主的板块
    }
...
function zib_bbs_apply_moderator_create($plate, $desc = '')
{
    ...
    if (_pz('email_bbs_apply_moderator_to_admin', true)) {
        //发送邮件
        @wp_mail(get_option('admin_email'), $mail_title, $msg_con . $mail_btn);
    }
