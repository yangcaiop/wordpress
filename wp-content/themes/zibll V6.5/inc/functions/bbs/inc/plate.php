 ...
 * @return {*}
 */
function zib_bbs_get_plate_edit_link($plate_id = 0, $cat_id = 0, $class = '', $con = '<i class="fa fa-edit"></i>板块设置', $tag = 'botton')
{
...
/**
 * @description: 根据排序方式获取全部板块分类列表
 * @param {*} $orderby 可选排序方式 last_reply:最后回复 | last_post:最后发帖 | count板块数量 | views:查看总数（热门）|name 名称
 * @param {*} $hide_empty 是否隐藏没有板块的分类
 * @return {*}
 */
...
/**
 * @description: 获取板块更多按钮的dropdown下拉框
 * @param {*} $term_id
 * @param {*} $class
 * @param {*} $con
 * @return {*}
...
function zib_bbs_plate_page_content()
{
    ...
    if ($is_hide) {
        echo '<div class="plate-tab zib-widget">' . zib_get_null('抱歉！您暂无查看此板块内容的权限', 10, 'null-cap.svg', '') . $is_hide . '</div>';
    } else {
