...
Zib_CFSwidget::create('zib_bbs_widget_ui_posts_lists', array(
    ...
    'fields'      => array(
        array(
            'label'   => '仅显示当前' . $zib_bbs->plate_name,
            'id'      => 'current_plate',
            'desc'    => '当此模块放置在' . $zib_bbs->plate_name . '页面的时候，开启此功能后，则按照当前' . $zib_bbs->plate_name . '进行筛选。可实现本版热门、本版精华等效果<div style="color: #ff6c6c;">开启此功能后，该模块只会在' . $zib_bbs->plate_name . '页面显示</div>',
            'type'    => 'switcher',
            'default' => false,
        ),
        array(
            ...
            'desc'        => '仅显示所选' . $zib_bbs->plate_name . '的帖子，支持单选、多选。输入板块关键词搜索选择',
            'default'     => '',
            'options'     => 'post',
            'query_args'  => array(
                'post_type' => 'plate',
...
function zib_bbs_widget_ui_posts_lists($args, $instance)
{
    ...
    if (!empty($instance['current_plate'])) {
        global $post;
        if (!isset($post->post_type) || 'plate' !== $post->post_type) {
            return;
        }
        $current_plate = $post->ID;
    }
