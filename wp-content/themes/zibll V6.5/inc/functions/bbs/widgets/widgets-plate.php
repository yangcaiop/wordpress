...
{
    global $post, $zib_bbs, $wp_query;
    $post_type = $wp_query->get('post_type');

    if ('plate' === $post_type) {
        $obj = $post;
    } else if ('forum_post' === $post_type) {
        $obj = get_post(zib_bbs_get_plate_id($post->ID));
    } else {
        return;
    }
