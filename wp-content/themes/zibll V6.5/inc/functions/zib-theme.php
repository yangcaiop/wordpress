...
function zib_baidu_resource_submission($url)
{
    ...
    //普通收录
    $response = $http->post($api, implode("\n", $urls));
    ...
    if (!is_admin()) {
        ...
        $jss = array(
            ...
            'bootcdn'    => array(
                ...
                'bootstrap' => '//cdn.bootcss.com/twitter-bootstrap/3.4.1/js/bootstrap.min.js',
            ),
            'he'         => array(
...
{
    $url = $url ? $url : home_url(add_query_arg(null, null));
    $url = urlencode($url);
    return esc_url(
        add_query_arg(
            array(
...
function zib_is_go_link($url)
{
    ...
    if ($go_link_exclude_domain) {
        ...
        if (in_array(zib_getTopHost($url), $exclude_domain)) {
            return false;
        }
...
add_action('wp_enqueue_editor', function () {
    ...
    $_pz_float_toolbar = (array) _pz('mce_float_toolbar', array('pc_s', 'm_s'));
...
function add_shortcode_hidecontent($atts, $content = null)
{
    ...
    if (apply_filters('hidecontent_is_show', false, $type)) {
        return '<div class="hidden-box show"><div class="hidden-text">本文隐藏内容</div>' . do_shortcode($content) . '</div>';
    }

    if ($user_id && $user_id == $post->post_author) {
        //作者直接显示内容
        return '<div class="hidden-box show"><div class="hidden-text">[' . $type_text[$type] . ']隐藏内容 - 作者可见</div>' . do_shortcode($content) . '</div>';
    }
    ...
    switch ($type) {
        ...
        case 'password':
            ...
            if ($pas && $pas == $password) {
                $html = '<div class="hidden-box show"><div class="hidden-text">此处隐藏内容</div>' . do_shortcode($content) . '</div>';
            } else {
