...
function zib_user_signtab_content($tab = 'signin', $is_page = false, $echo = true)
{
    ...
    if (zib_is_oauth_qrcode_s()) {
        ...
        $tab_html .= '<span class="muted-2-color px12">使用<a class="muted-color" href="#tab-sign-in" data-toggle="tab">其它方式登录</a>或<a class="muted-color" href="#tab-sign-up" data-toggle="tab">注册</a></span>';
...
function zib_signup_form($type = '')
{
    ...
    $input .= '<button type="button" class="but radius jb-green padding-lg signsubmit-loader btn-block">' . zib_get_svg('signup', '0 0 1024 1024', 'icon mr10') . '注册</button>';
...
function zib_get_user_name($args = array())
{
    ...
    $html      = '<name class="' . $args['class'] . '">' . $name_link . $auth . $level . $follow . '</name>';
