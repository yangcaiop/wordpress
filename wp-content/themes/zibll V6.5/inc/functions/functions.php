...
function zib_new_slider($args, $echo = true)
{
    ...
    foreach ($args['slides'] as $slide) {
        ...
        //背景图
        $s_background = '<img class="lazyload swiper-lazy radius8" ' . ($lazy ? ' data-src="' . $s_background . '" src="' . $lazy_src . '"' : ' src="' . $s_background . '"') . ' alt="' . $img_alt . '">';
        ...
        if (!empty($slide['image_layer'][0]['image'])) {
            foreach ($slide['image_layer'] as $layer) {
                ...
                $layer_image = $layer_image ? '<img class="lazyload swiper-lazy radius8" ' . ($lazy ? ' data-src="' . $layer_image . '" src="' . $lazy_src . '"' : ' src="' . $layer_image . '"') . ' alt="' . $img_alt . '">' : '';
...
function zib_get_user_badges($user_id)
{
    ...
    $followed_n = _cut_count(get_user_meta($user_id, 'followed-user-count', true));
...
function zib_posts_related($related_title = '相关阅读', $limit = 6)
{
    global $post;
...
/**
 * @description: 获取帖子或者板块状态post_status的徽章
 * @param {*} $class
 * @param {*} $posts_id
 * @return {*}
 */
...
function zib_modal($args = array())
{
    ...
    $close_btn = '<button class="close" data-dismiss="modal">
...
if (!isset($_COOKIE["showed_weixingzh_auto"]) && _pz('weixingzh_auto') && zib_is_wechat_app() && !get_current_user_id() && zib_get_oauth_login_url('weixingzh')) {
    add_action('wp_footer', 'zib_weixingzh_auto', 10);
}

function zib_weixingzh_auto()
{
...
function zib_query_orderby_filter($orderby, $args = array())
{
    ...
    if (in_array($orderby, $mate_orderbys_num)) {
        $args['orderby']  = 'meta_value_num modified';
        $args['meta_key'] = $orderby;
    } elseif (in_array($orderby, $mate_orderbys)) {
        $args['orderby']  = 'meta_value modified';
        $args['meta_key'] = $orderby;
    } else {
...
function zib_graphic_card($args = array(), $echo = false)
{
    ...
    $img      = '<img class="lazyload fit-cover" ' . ($lazy ? ' alt="' . ($args['alt'] ?: '图片') . zib_get_delimiter_blog_name() . '" data-src="' . $args['img'] . '" src="' . $lazy_src . '"': ' src="' . $args['img'] . '"') . '>';
 ...
 * @return {*}
 */
function zib_get_ajax_number_paginate($count_all, $paged = 1, $ice_perpage = 10, $ajax_url = '', $pag_class = 'ajax-pag', $next_class = 'next-page ajax-next')
{
    $args = array(
        'url_base'     => add_query_arg(array('paged' => '%#%'), $ajax_url), // http://example.com/all_posts.php%#% : %#% 替换为页码。
...
function zib_get_ias_ajaxpager($args)
{
    ...
    $html .= '<span class="post_ajax_trigger hide"><a' . ('load' == $args['type'] ? ' load-click' : '') . ' ajax-href="' . esc_url($args['url']) . '" class="ajax_load ajax-next ajax-open">asdasd</a></span>';
...
function zib_get_main_tab_nav($type = 'nav', $tabs_options, $id_prefix = 'home', $is_mobile_swiper = true, $active_key_str = 'tab', $route = false)
{
    ...
    $ajax_url = zib_url_del_paged(home_url(add_query_arg(null, null)));
...
function zib_get_cfs_icon($val)
{
    ...
    if (stristr($val, 'zibsvg-')) {
        return zib_get_svg(str_replace('zibsvg-', '', $val));
    }
