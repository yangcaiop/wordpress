...
add_filter('main_msg_tab_nav_content', function ($html) {
...
add_filter('main_msg_tab_content_content', function ($html) {
...
add_filter('main_msg_tab_content_posts', function () {
    return zib_get_user_cat_msg('posts');
});
add_filter('main_msg_tab_content_like', function () {
    return zib_get_user_cat_msg('like');
});
add_filter('main_msg_tab_content_system', function () {
    return zib_get_user_cat_msg('system');
});
add_filter('main_msg_tab_content_private', function () {
...
function zib_get_user_msg_lists($user_id = '', $where, $ajax_url = '', $cat = 'news')
{
    ...
    if ($count_all && $db_msg) {
        ...
        $ajax_url = $ajax_url ? $ajax_url : home_url(add_query_arg(null, null));
...
function zib_msg_set_user_page_tabs_array($tabs_array)
{
    ...
    $account = $tabs_array['account'];
    unset($tabs_array['account']);

    $tabs_array['msg'] = array(
        'title'    => zib_get_user_page_nav_title(zib_get_svg('msg-color', null, 'icon fa-fw mr10 em16') . '消息通知'),
