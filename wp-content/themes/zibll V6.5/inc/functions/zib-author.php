...
function zib_author_header()
{
    ...
    $name       = '<div class="em12 name">' . zib_get_user_name("id=$author_id&vip=1&auth=0") . '</div>';
...
function zib_get_author_header_metas($author_id)
{
    ...
    $followed_n = get_user_meta($author_id, 'followed-user-count', true);
...
function zib_main_author_tab_content_favorite()
{
    ...
    return zib_get_favorite_posts_lists($author_id);
}
...
function zib_get_follow_user_list($user_id, $type = 'followed', $paged = 1)
{
    ...
    $follow_count   = is_array($follow) ? count($follow) : 0;
    $followed_count = is_array($followed) ? count($followed) : 0;
...
function zib_main_author_tab_content_post()
{
    ...
    $header   = '';
    $this_url = home_url(add_query_arg(array()));

    if (1 == $page || !_pz('paging_ajax_s', true)) {
        $this_url = zib_url_del_paged(home_url(add_query_arg(array())));
