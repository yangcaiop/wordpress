...
function zibpay_get_order_effective_amount($order)
{
    ...
    if ($pay_detail && is_array($pay_detail)) {
        $price  = 0;
        $method = array('wechat', 'alipay', 'balance'); //哪些支付方式是有效的
        foreach ($method as $t) {
...
{
    $methods    = zibpay_get_payment_method_names();
    $order      = (array) $order;
    $pay_detail = maybe_unserialize($order['pay_detail']);
    $lists      = '';
    $i          = 1;
    foreach ($methods as $k => $v) {
        if (!empty($pay_detail[$k])) {
            $lists .= $i !== 1 ? $separator : '';
            $lists .= '<lists class="' . $class . '">' . $v . '：' . $pay_detail[$k] . '</lists>';
            $i++;
        }
    }
    if (!$lists) {
...
function zibpay_get_payment_method_names()
{
...
//支付方式
function zibpay_get_payment_methods()
{
    $methods        = array();
...
function zibpay_get_initiate_pay_input($pay_type, $pay_price = 0, $text = '立即支付')
{
    ...
    $methods_lists   = '';
    $method_args     = array(
        'wechat'  => '<img src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/assets/img/pay-wechat-logo.svg" alt="wechat-logo"><div>微信支付</div>',
        'alipay'  => '<img src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/assets/img/pay-alipay-logo.svg" alt="alipay-logo"><div>支付宝支付</div>',
        'balance' => '<img src="' . ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/assets/img/pay-balance-logo.svg" alt="balance-logo"><div>余额支付</div>',
    );

    if (!zibpay_is_allow_balance_pay($pay_type)) {
        $balance_key = array_search('balance', $payment_methods);
        if ($balance_key && isset($payment_methods[$balance_key])) {
            unset($payment_methods[$balance_key]);
        }
    }
    ...
    $method_default = $payment_methods[0];
    if (count($payment_methods) > 1) {
        ...
        foreach ($payment_methods as $method) {
            ...
            $methods_lists .= '<div class="flex jc hh payment-method-radio hollow-radio flex-auto pointer' . ($ii === 1 ? ' active' : '') . '" data-for="payment_method"  data-value="' . $method . '" >' . $method_args[$method] . '</div>';
            $ii++;
        }
    }
    if ($methods_lists) {
        $methods_lists = '<div class="muted-2-color em09 mb6">请选择付款方式</div><div class="flex ac mb10">' . $methods_lists . '</div>';
    }

    //如果存在余额支付，则需显示我的余额
    $user_balance_box = '';
    if (in_array('balance', $payment_methods)) {
...
function zibpay_get_order_statistics_totime($time_type = 'today')
{
    ...
    $data = $wpdb->get_row("SELECT COUNT(*) as count,SUM(pay_price) as sum FROM {$wpdb->zibpay_order} WHERE pay_time LIKE '%$like_time%' and status=1 and pay_price > 0 and order_type != 8");
    $data = (array) $data;
    if (!isset($data['count'])) {
