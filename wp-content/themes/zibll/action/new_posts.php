...
function zib_ajax_new_posts()
{
    ...
    if ('posts_save' == $action) {
        ...
        if (_new_strlen($title) > $title_strlen_limit['max']) {
            zib_send_json_error('标题太长了，不能超过30个字');
        }
        if (_new_strlen($title) < $title_strlen_limit['min']) {
...
function zib_ajax_newpost_save_pay($post_id = 0, $echo_success = true)
{
    ...
    if ($zibpay_s) {
        ...
        if ($pay_modo !== 'points') {
            ...
            if (!$pay_price) {
                zib_send_json_error(array('posts_id' => $post_id, 'msg' => '请设置阅读限制的支付金额'));
            }
            ...
            if (!$points_price) {
                zib_send_json_error(array('posts_id' => $post_id, 'msg' => '请设置阅读限制的积分数量'));
            }
    ...
    $pay_data = array_merge(
        array(
            ...
            'pay_cuont'           => 0,
            'pay_doc'             => !empty($posts_zibpay['pay_doc']) ? strip_tags(trim($posts_zibpay['pay_doc'])) : '',
        ), $pay_data);
