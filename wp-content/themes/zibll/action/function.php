...
function zib_post_views_record()
{
    ...
    $poat_id = !empty($_REQUEST['id']) ? $_REQUEST['id'] : 0;
    if ($poat_id) {
        $post_views = (int) get_post_meta($poat_id, 'views', true);
        $new        = $post_views + 1;
        update_post_meta($poat_id, 'views', $new);
        do_action('posts_views_record', $poat_id, $new);
        wp_send_json_success(array('poat_id' => $poat_id, 'views' => $new));
    }
...
function zib_ajax_poster_share_data()
{
    ...
    if (stristr($id, 'term')) {
        ...
        $banner = zib_get_taxonomy_img_url($term->term_id, 'full', '');
        ...
        $banner = esc_url(zib_post_thumbnail('full', '', true, $post));
