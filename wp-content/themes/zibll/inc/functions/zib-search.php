...
//搜索页面的主要内容函数
function zib_search_content($s, $type = 'post', $cat = '', $user = '') {
    ...
    if ($s) {
        ...
        foreach ($type_name_array as $k => $v) {
            ...
            $href  = zib_url_del_paged(home_url(add_query_arg(array('type' => $k, 'cat' => $cat, 'user' => $user))));
...
function zib_save_history_search($s) {
    $s = esc_attr(strip_tags($s));
    if (_new_strlen($s) >= 2 && _new_strlen($s) < 9) {
