...
//添加经验值统一接口,数量
function zib_add_user_level_integral($user_id = 0, $value = 0, $key = '')
{
    //判断用户还能增加经验值，允许。||是否超过今日上限
    if (!$user_id || !$value || !zib_user_is_allow_add_integral($user_id)) {
        return;
    }
...
class zib_user_level_integral_add
{
    public function __construct()
    {
        ...
        add_action('plate_is_hot', array($this, 'bbs_plate_hot')); //热门板块
    ...
    public function sign_up($user_id)
    {
        ...
        if ($value) {
            zib_add_user_level_integral($user_id, $value, 'sign_up');
        }
    }
