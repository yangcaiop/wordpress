            ...
            <form>
                <div class="scroll-y mini-scrollbar max-vh5">
                    <div class="mb10 muted-box padding-10 em09">
                        <div class="flex"><div class="author-set-left" style="min-width: 80px;">用户名</div><div class="author-set-right mt6">' . zib_get_user_name("id=$user_id&vip=0") . '</div></div>
...
function zib_get_ban_info_modal($user_id = 0)
{
    ...
    if (!$is_ban) {
        ...
        $con = '
                <div class="mb20">
                    <div class="mb10 flex"><div class="author-set-left" style="min-width: 80px;">用户名</div><div class="author-set-right mt6">' . zib_get_user_name("id=$user_id&vip=0") . '</div></div>
...
function zib_get_ban_log_modal($user_id = 0)
{
    ...
    $html = '
    <div class="flex ac mb6">' . zib_get_avatar_box($user_id, 'avatar-mini mr6') . zib_get_user_name("id=$user_id&vip=0") . '</div>
    ' . $desc . '
...
function zib_get_set_user_ban_modal($user_id = 0)
{
    ...
    if ($is_ban) {
        ...
        $html .= '
                <div class="mb20">
                    <div class="mb10 flex"><div class="author-set-left" style="min-width: 80px;">用户</div><div class="author-set-right mt6">' . zib_get_user_name("id=$user_id&vip=0&class=inflex ac badg p2-10 em09") . '</div></div>
    ...
    $header  = '<div class="mb10 edit-header touch"><button class="close" data-dismiss="modal"><svg class="icon ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button><b class="flex ac c-red"><span class="toggle-radius mr10 c-red"><i class="fa fa-ban"></i></span>禁封用户</b></div>';
    $options = array(
        '1' => array(
            'name' => '拉入小黑屋',
...
if (_pz('user_ban_s', true)) {
    ...
    function zib_ban_page_header_desc_filter($desc, $user_id)
    {
        $ban_badge = zib_get_user_ban_badge($user_id, 'img-icon mr3');
        if ($ban_badge) {
            $ban_badge = zib_get_user_ban_info_link($user_id, 'but p2-10 em09 mt6 mr6', $ban_badge . '小黑屋禁封中 <i class="ml6"><i class="fa fa-angle-right em12"></i></i>');
...
function zib_get_report_modal($user_id, $url = '')
{
    ...
    if ($ing) {
        ...
        if (get_current_user_id()) {
            ...
            $con .= '
                    <div class="mb10 muted-box padding-10 em09">
                        <div class="flex"><div class="author-set-left" style="min-width: 80px;">举报用户</div><div class="author-set-right mt6">' . zib_get_user_name("id=$report_user_id&vip=0") . '</div></div>
    ...
    $action = 'report_user';
    $header = '<div class="mb10 edit-header touch"><button class="close" data-dismiss="modal"><svg class="icon ic-close" aria-hidden="true"><use xlink:href="#icon-close"></use></svg></button><b class="flex ac c-red"><span class="toggle-radius mr10 c-red"><i class="fa fa-exclamation-triangle"></i></span>举报用户</b></div>';
