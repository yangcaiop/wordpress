...
function zib_menu_button($layout = 1)
{
    ...
    $radius_but = in_array('pc_nav', (array) _pz('theme_mode_button', array('pc_nav', 'm_menu'))) ? '<a href="javascript:;" class="toggle-theme toggle-radius"><i class="fa fa-toggle-theme"></i></a>' : '';
...
function zib_header_user_box()
{
    ...
    if ($user_id) {
        ...
        $href .= '<a href="javascript:;" class="signup-loader"><div class="badg mb6 toggle-radius c-green">' . zib_get_svg('signup') . '</div><div class="c-green">注册</div></a>';
        $href .= '<a target="_blank" href="' . add_query_arg('redirect_to', home_url(add_query_arg(null, null)), zib_get_sign_url('resetpassword')) . '"><div class="badg mb6 toggle-radius c-purple">' . zib_get_svg('user_rp') . '</div><div class="c-purple">找回密码</div></a>';
        $href .= '</div>';
...
function zib_nav_mobile($location = 'mobilemenu')
{
    ...
    $menu .= in_array('m_menu', (array) _pz('theme_mode_button', array('pc_nav', 'm_menu'))) ? '<a href="javascript:;" class="toggle-theme toggle-radius"><i class="fa fa-toggle-theme"></i></a>' : '';
