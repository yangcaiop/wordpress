...
function zib_bbs_get_home_tab_content_plate($html = '', $option = array())
{
    ...
    $can_plate_add = zib_bbs_current_user_can('plate_add'); //判断是否有添加板块的权限)
    ...
    if (!$html) {
        $html = zib_get_null('暂时没有板块', 60, 'null.svg', '', 0, 170);
...
function zib_bbs_get_home_tab_content_follow_plate($html = '', $option)
{
    ...
    $title = !empty($option['user_follow']['title']) ? $option['user_follow']['title'] : '我关注的板块';
...
function zib_bbs_get_home_tab_content_follow_tab_plate($html = '', $option = array())
{
    ...
    if ($user_id) {
        ...
        if ($follow_plate) {
            $args['post__not_in'] = $follow_plate; //排除自己已经关注的板块
        }
    }
