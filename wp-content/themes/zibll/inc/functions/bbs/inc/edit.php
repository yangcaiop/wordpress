...
class zib_bbs_edit
{
    ...
    public static function submit_text($inargs = array())
    {
        $submit_text = '';
        ...
        if ($inargs['post_id']) {
            if ($inargs['post_author'] && get_current_user_id() != $inargs['post_author']) {
                ...
                $submit_text = '<div class="muted-2-color mt6 mr10">正在修改[' . zib_get_user_name("id=$post_author&auth=0&level=0&class=&name_class=focus-color") . ']提交的内容</div>';
            }
    ...
    public static function plate($plate_id, $cat_id = 0)
    {
        ...
        if ((!$plate_id && zib_bbs_current_user_can('plate_set_allow_view')) || ($plate_id && zib_bbs_current_user_can('plate_set_allow_view', $plate_id))) {
            ...
            $allow_view_set = $allow_view_set ? '<div class="mt20 dependency-box"><div class="em09 muted-2-color mb6">查看权限</div>' . $allow_view_set . '<div class="px12 muted-3-color mb6">为此' . $name . '设置阅读限制，只有满足条件的用户才能查看此板块的' . $zib_bbs->posts_name . '内容</div></div>' : '';
        }
