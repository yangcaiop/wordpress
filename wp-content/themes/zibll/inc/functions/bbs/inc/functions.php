...
function zib_bbs_get_paginate($count_all, $page = 0, $ice_perpage = 0, $ajax_url = false, $type = null)
{
    ...
    if (!$ajax_url) {
        $ajax_url = home_url(remove_query_arg('paged'));
...
/**
 * @description: 获取板块或者帖子的热门徽章
 * @param {*} $id int 帖子或者板块的id
 * @param {*} $class
 * @param {*} $tag
 * @return {*}
 */
...
function zib_bbs_updata_views($post_id = 0)
{
    ...
    if ($post_id && 'forum_post' == $post_type) {
        //更新当前帖子的标签以及话题的总阅读数量
        $terms = get_the_terms($post_id, ['forum_tag', 'forum_topic']);
        if ($terms) {
            foreach ($terms as $term) {
                $count = (int) get_term_meta($term->term_id, 'views', true);
                $count++;
                wp_cache_set($term->term_id, $count, 'bbs_term_views_count');
                update_term_meta($term->term_id, 'views', $count);
            }
        }
        ...
        if ($plate_id) {
            //更新板块的总查看数量
            $count = (int) get_post_meta($plate_id, 'views', true);
...
function zib_bbs_updata_is_hot($post_id = 0)
{
    ...
    try {
        ...
        $plate_views   = get_post_meta($plate_id, 'views', true);
        $posts_count   = zib_bbs_get_plate_posts_count($plate_id);
        $posts_opt     = _pz('is_hot_posts');

        $is_hot = ($views > $posts_opt['views'] && $comment_count > $posts_opt['comment'] && ($views > ($plate_views / $posts_count * (1 + $posts_opt['average']))));

        $is_hot = ($is_hot ? 1 : 0);
        $is_hot = apply_filters('bbs_is_hot_posts', $is_hot, $post);

        update_post_meta($post_id, 'is_hot', $is_hot);
        ...
        if (isset($plate_cat[0])) {
            ...
            $cat_views       = get_term_meta($plate_cat_id, 'views', true);
            ...
            $is_hot = ($p_posts_count > $p_opt['posts_count'] && $plate_views > $p_opt['views'] && $p_comment_count > $p_opt['comment'] && ($plate_views > ($cat_views / $posts_count * (1 + $p_opt['average']))));
            $is_hot = ($is_hot ? 1 : 0);
            $is_hot = apply_filters('bbs_is_hot_plate', $is_hot, $post);

            update_post_meta($plate_id, 'is_hot', $is_hot);
            if ($is_hot) {
                do_action('plate_is_hot', $post);
            }
