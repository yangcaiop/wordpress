...
function zib_bbs_user_can($user_id, $capability, ...$args)
{
    ...
    switch ($capability) {
        ...
        case 'plate_edit': //编辑板块
        case 'posts_delete': //删除帖子
        case 'plate_delete': //删除板块
        case 'posts_plate_move': //移动板块
        case 'plate_plate_cat_edit': //为自己创建的板块切换板块分类
        ...
        case 'question_answer_adopt': //采纳回答
            ...
            if (isset($post->post_author) && $post->post_author == $user_id) {
                ...
                //如果自己不是作者，则检查用户是否是该板块或者帖子的版主、超级版主、管理员
        ...
        case 'plate_cat_set_add_limit': //板块分类设置板块创建限制
            if (empty($args[0])) {
                //没有板块分类的ID，则为新建板块
                if (zib_is_can_roles($user_id, $cap_roles)) {
                    return true;
        ...
        case 'plate_set_add_limit': //板块设置发贴限制
        ...
        //term的权限
        case 'plate_cat_edit': //编辑板块分类
        case 'plate_cat_delete': //编辑板块分类
        ...
        case 'plate_set_add_limit_other'; //为自己管理的板块设置[发帖限制]
        case 'plate_set_allow_view_other'; //为自己管理的板块设置[查看权限]
        case 'posts_allow_view_points_other'; //为自己管理的板块设置[积分支付查看]
        case 'posts_allow_view_pay_other'; //为自己管理的板块设置[付费查看]
...
//等级经验值参数的定义
add_filter('integral_add_options', function ($options) {
    global $zib_bbs;
    ...
    $bbs = array(
        ...
        'bbs_plate_hot'   => array($zib_bbs->plate_name . '成为热门', 10, '创建的' . $zib_bbs->plate_name . '成为热门板块', '论坛'),
    );
