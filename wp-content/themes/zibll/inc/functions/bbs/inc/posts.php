...
function zib_bbs_get_posts_mini_list($class = 'ajax-item', $show_topping = false)
{
    ...
    if ($last_reply) {
        $time = '<span class="icon-circle" title="最后回复：' . $last_reply . '">最后回复于' . zib_get_time_ago($last_reply) . '</span>';
    } else {

        $time = '<span class="icon-circle" title="发布时间：' . $get_the_time . '">发布于' . $get_the_time_ago . '</span>';
    }
...
function zib_bbs_get_posts_lists_content($post = null, $show_topic = true)
{
    ...
    //图像
    $imgs      = zib_bbs_get_posts_lists_img($post); //图像s
    $imgs_html = '';
    if ($imgs) {
        $imgs_count = zib_get_post_imgs_count($post);
        $imgs_html  = '<div class="lists-imgs mt6 count-' . ($imgs_count <= 4 ? $imgs_count : 4) . '">' . $imgs . '</div>';
    }
...
function zib_bbs_get_allow_view_data($post = null)
{
    ...
    switch ($allow_view) {
        ...
        case 'points':
            ...
            if (!empty($pay_mate['pay_type']) && 'no' !== $pay_mate['pay_type']) {
                ...
                if (!$paid) {
                    ...
                    $con   = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">登录后继续查看</p><p><a href="javascript:;" class="signin-loader but jb-blue padding-lg"><i class="fa fa-fw fa-sign-in" aria-hidden="true"></i>登录</a><a href="javascript:;" class="signup-loader ml10 but jb-yellow padding-lg">' . zib_get_svg('signup') . '注册</a></p></div>';
                    $con   = zib_bbs_get_posts_pay_box($posts_id);
        ...
        case 'comment':
            ...
            if (!$user_id) {
                $title = '内容已隐藏，请评论后查看';
                $con   = '<div class="text-center em09 mt20"><p class="separator muted-3-color mb20">登录后继续评论</p><p><a href="javascript:;" class="signin-loader but jb-blue padding-lg"><i class="fa fa-fw fa-sign-in" aria-hidden="true"></i>登录</a><a href="javascript:;" class="signup-loader ml10 but jb-yellow padding-lg">' . zib_get_svg('signup') . '注册</a></p></div>';
        ...
        case 'roles':
            ...
            if (!$user_id) {
                ...
                $con .= $roles;
                $con .= '<p class="separator muted-3-color mb20 mt20">登录后查看我的权限</p><p><a href="javascript:;" class="signin-loader but jb-blue padding-lg"><i class="fa fa-fw fa-sign-in" aria-hidden="true"></i>登录</a><a href="javascript:;" class="signup-loader ml10 but jb-yellow padding-lg">' . zib_get_svg('signup') . '注册</a></p>';
                $con .= '</div>';
            } else {
                $is_allow = false;
 ...
 * @return {*} zib_get_refresh_modal_link()
 */
function zib_bbs_get_posts_plate_move_link($posts_id = 0, $class = '', $con = '<i class="fa fa-trash-o fa-fw"></i>移动板块', $tag = 'a')
{
 ...
 * @return {*}
 */
function zib_bbs_get_posts_lists_img($post = null)
{
    ...
    if (false === $cache_html) {
        ...
        $html  = zib_get_post_imgs($post, _pz('bbs_thumb_size'), $class, 4, true);
