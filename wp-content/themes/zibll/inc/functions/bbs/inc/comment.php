...
function zib_bbs_get_respond()
{
    ...
    if (!$user_id) {
        ...
        $html .= '<a href="javascript:;" class="signup-loader ml10 but c-yellow padding-lg"><i data-class="icon mr10" data-viewbox="0 0 1024 1024" data-svg="signup" aria-hidden="true"></i>注册</a>';
        $html .= '</p>';
...
function zib_bbs_get_comment_title()
{
    ...
    global $wp_rewrite, $authordata, $post, $zib_bbs;
    $url        = home_url(add_query_arg('cpage', false)); //筛选 过滤
...
function zib_bbs_get_answer_adopt_modal($comment_id)
{
    ...
    $form .= '<div class="mt20 but-average">
                    ...
                    <button class="but c-blue wp-ajax-submit"><i class="fa fa-check" aria-hidden="true"></i>确认采纳</button>
                </div>';
    $form .= '</form>';
