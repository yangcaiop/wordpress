...
function zib_bbs_ajax_follow_plate()
{
    ...
    //授权判断
    $home_url = home_url();
    if (!(stristr($home_url, 'localhost') || stristr($home_url, '127.') || stristr($home_url, '192.')) && !zib_is_aut()) {
...
function zib_bbs_ajax_edit_posts()
{
    ...
    if (_new_strlen($post_title) > $title_strlen_limit['max']) {
        zib_send_json_error('标题太长了，不能超过30个字');
    }

    if ('bbs_posts_save' == $action) {
        //提交保存、审核、发布
        if (_new_strlen($post_title) < $title_strlen_limit['min']) {
...
function zib_bbs_add_posts_pending_msg($post)
{
    ...
    if (_pz('email_bbs_posts_pending_to_admin', true)) {
        @wp_mail(get_option('admin_email'), $email_title, $message);
    }
...
function zib_bbs_ajax_edit_allow_view($post_id = 0, $echo_success = true)
{
    ...
    switch ($allow_view) {
        ...
        case 'points':
            ...
            if (!$points_price) {
                zib_send_json_error(array('post_id' => $post_id, 'msg' => '请设置阅读限制的积分数量'));
            }
