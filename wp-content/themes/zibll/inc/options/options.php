...
function _pz($name, $default = false, $subname = '')
{
    ...
    if ($zib_get_option) {
        ...
        $zib_get_option = get_option('zibll_options');
    }
...
//更新主题时候重置用户权限
function zib_update_user_cap()
{

    //重置用户权限
    foreach (array('subscriber', 'contributor') as $user_role) {
...
add_action("zibll_update_notices", 'zib_update_user_cap');
