...
add_filter('initiate_order_data', 'zibpay_initiate_order_data');
...
function zibpay_initiate_pay($order_data)
{
    ...
    /**准备付款接口 */
    $pay_sdk = '';
    if ('balance' == $order_data['payment_method']) {
        $pay_sdk = 'balance';
    } elseif ('wechat' == $order_data['payment_method']) {
    ...
    if (!$pay_sdk || 'null' == $pay_sdk) {
        return array('error' => 1, 'msg' => '网站未接入此收款方式，请联系客服');
    }
